    ...
    <nav className="bg-[#0D3C1D] p-2 px-6 md:px-16 flex items-center justify-between fixed w-full z-50">
      ...
      { home ? <div>
        <Image src={Menu} alt='menu' width={24} height={24} className='block md:hidden' onClick={showNav}/>
          <Navbar closeNav={hideNav} handleScroll={handleScroll} about={about} classNames={nav ? "block h-full shadow shadow-xl z-30" : "hidden"} className="text-end mt-5 mb-3 self-end mr-4"  hideNav={hideNav}/>
           </div>
